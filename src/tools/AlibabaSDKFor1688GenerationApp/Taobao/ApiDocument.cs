...
//            //if (isFull)
//            //{
//            //    var publicAllApis = await TaobaoDataCache.ListPublicApiByCacheAsync();
...
//            //    publicApiDetails = publicApis.Select(async f => new eeeeeeeee { ApiInfo = f, ApiDetail = await TaobaoDataCache.GetApiDetailByCacheAsync(f.Namespace, f.Name, f.Version) })/*.Skip(587).Take(1)*/.ToList();
//            //}
//            //else
//            //{
//            var apiCategorys = await TaobaoDataCache.GetAllApiCategoryAsync();
//            publicApiDetails = apiCategorys.OrderBy(f => f.TreeName).SelectMany(f => f.CatelogList.OrderBy(ff => ff.Name).Select(async ff => new eeeeeeeee
//            {
//                ApiInfo = new Datum { TreeName = f.TreeName, Tips = ff.Tips, DocId = ff.DocId, DocType = ff.DocType, Name = ff.Name },
//                ApiDetail = await TaobaoDataCache.GetDocumentByCacheAsync(ff.DocId, ff.DocType)
//            })).ToList();
//            //}
...
//        public Models.DocumentData ApiDetail { get; set; }
//    }
